  ...
  useEffect(() => {
    const fetchFileData = async () => {
      try {
        ...
        const response = await fetch(`https://api.github.com/repos/${repository}/contents/${filePath}?ref=${branch}`);
        if (!response.ok) {
    ...
    return (
      <div className={`my-8 ${className}`}>
        <div className="bg-red-50 border border-red-200 rounded-lg p-6">
          ...
          <p className="text-red-600 text-sm">{error || 'File not found'}</p>
        ...
        <div className="border-b border-gray-200 p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <span className="font-mono text-lg font-medium text-gray-900">{filename}</span>
            ...
            <div className="flex items-center space-x-4 text-sm text-gray-500">
              ...
              <a
                ...
                className="bg-gray-100 text-gray-700 px-3 py-1 rounded hover:bg-gray-200 transition-colors text-sm font-medium"
              >
                View on GitHub →
              </a>
            </div>
